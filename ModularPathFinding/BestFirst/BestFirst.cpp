...
std::vector<NODE> PathAlgorithms::BestFirst::GeneratePath(glm::ivec2 _start, glm::ivec2 _goal, std::vector<std::vector<NODE*>> _map, bool _allowDiagonal)
{
	...
	while (open.size() != 0)
	{
		...
		for each (NODE* neighbour in neighbours)
		{
			...
			{
				closed.clear();
				return CleanUp(neighbour, index, open); //, closedList, openList_vec, closedList_vec);
			}
